...
scheduler = ReduceLROnPlateau(optimizer,mode='max', patience=2, factor=0.2, threshold=0.01)  # 学习率调度器，使用L2正则化进行权重衰减,min模式表示损失指标越小越好
loss_fn = torch.nn.BCEWithLogitsLoss()  # 可以根据类别权重调整此损失函数

# 记录文件路径
log_file_path = 'D:\\Experiment_zyj\\Python_exp\\policy_map\\instrument_bert_迭代过程.txt'
...
# 保存图像
plt.savefig('D:\\Experiment_zyj\\Python_exp\\policy_map\\instrument_bert_曲线图.png')
#plt.show()
print(f"曲线图已保存")

# 保存模型权重
model_save_path = 'D:\\Experiment_zyj\\Python_exp\\policy_map\\instrument_bert_model.pth'
