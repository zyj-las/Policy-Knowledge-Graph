...
# 读取Excel数据
train_data = pd.read_excel('D:\\File_zyj\\2.论文文件\\4.小论文_政策知识图谱\\数据分析文件0929\\作用行为-训练.xlsx')

train_texts = train_data['政策段落'].tolist()
train_labels = train_data['作用行为'].apply(lambda x: x.split(',')).tolist()

# 标签编码
mlb = MultiLabelBinarizer(classes=['ab1', 'ab2', 'ab3', 'ab4'])
...
# 记录文件路径
log_file_path = 'D:\\Experiment_zyj\\Python_exp\\policy_map\\actbe_distilbert_迭代过程.txt'
...
bestmodel_save_path = 'D:\\Experiment_zyj\\Python_exp\\policy_map\\actbe_distilbert_model_best.pth'  # 最佳模型保存路径
'''
# 训练和验证循环
...
# 保存图像
plt.savefig('D:\\Experiment_zyj\\Python_exp\\policy_map\\actbe_distilbert_曲线图.png')
#plt.show()
print(f"曲线图已保存")

# 保存最终模型权重
model_save_path = 'D:\\Experiment_zyj\\Python_exp\\policy_map\\actbe_distilbert_model.pth'
...
print(f"作用行为模型已保存到 {model_save_path}")
